import os
os.environ["CUDA_VISIBLE_DEVICES"] = "1" # Change for the GPU to use
...
parser.add_argument(
    ...
    type=int,
    default=2,
    help="Number of layers in the QML model (default: 2)"
)
parser.add_argument(
    "-r", "--runs",
    type=int,
    default=3,
    help="Number of runs for the experiment (default: 3)"
)
...
best_model_prefix = "qmlp-CIFAR-run"
cm_name_prefix= "cm_qmlp-CIFAR-run"
filename_prefix = "qmlp-CIFAR-run"
noise_afterfix = ""
...
def unpickle(file):
    ...
    return dict_data
def load_cifar100(data_dir, use_coarse=False):
    # Load train
    train_dict = unpickle(os.path.join(data_dir, "train"))
    X_train = train_dict[b'data']
    y_train = train_dict[b'coarse_labels'] if use_coarse else train_dict[b'fine_labels']

    test_dict = unpickle(os.path.join(data_dir, "test"))
    X_test = test_dict[b'data']
    y_test = test_dict[b'coarse_labels'] if use_coarse else test_dict[b'fine_labels']
    ...
    coarse_label_names = [l.decode("utf-8") for l in meta[b'coarse_label_names']]

    return (X_train, np.array(y_train)), (X_test, np.array(y_test)), fine_label_names, coarse_label_names


(X_train, y_train), (X_test, y_test), fine_labels, coarse_labels = load_cifar100("../datasets/cifar-100-python")
