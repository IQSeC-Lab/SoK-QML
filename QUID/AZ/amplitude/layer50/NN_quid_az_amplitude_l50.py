"""
QUID_QDetection_AZ23_Amplitude_l50.py
============================================================
Combines:
...
Optimisations over naive implementation:
  1. PROTOTYPE CACHE  (replaces full density-matrix cache)
     Instead of caching all N training density matrices (~770 GB
     for 184k samples), only the 23 per-class mean density matrices
     (prototypes) are cached (~96 MB total).  Prototypes are computed
     ONCE at startup using all available CPU cores (joblib), saved to
     rho_proto_cache_amplitude.npy, and reused across every QUID call.
     Re-computation is skipped on subsequent runs if the cache
     file already exists.

     Trade-off: prototypes are built from the FULL training set rather
     than the clean subset.  At N=184k the effect is negligible —
     poisoning even 50% of samples shifts per-class means by < 1%.
  ...
  2. MULTI-GPU PARALLEL EXPERIMENTS
     ...
     GPUs are used, e.g.:
       CUDA_VISIBLE_DEVICES=0,1,2,3 python QUID_QDetection_AZ23_Amplitude_l50.py

Model architecture (identical to NN_AZ_amplitude_l50_LF.py):
...
Output folder structure:
  ...
  runs/
    └── quid_qdetection_AZ23_amplitude_l50_summary.csv
  rho_proto_cache_amplitude.npy  (cached class prototypes, shape [23, 512, 512], ~96 MB)
============================================================
"""
...
ENCODING             = "Amplitude"
RHO_PROTO_CACHE_FILE = "rho_proto_cache_amplitude.npy"  # separate cache from angle version

RUN_DIR     = "runs"
os.makedirs(RUN_DIR, exist_ok=True)
SUMMARY_CSV = os.path.join(RUN_DIR, "quid_qdetection_AZ23_amplitude_l50_summary.csv")

# Q-Detection subsampling — keeps all 23 classes but limits samples per class.
# This controls the J matrix size: N_total = N_CLASSES * QDETECT_SUBSET_PER_CLASS
# e.g. 23 * 700 = 16100 samples → 16100x16100 J matrix (~2 GB) instead of 271 GB
QDETECT_SUBSET_PER_CLASS = 700   # ~16k total, adjust lower if still OOM
...
# ============================================================
# 1. Data  — matches NN_AZ_amplitude_l2_LF.py exactly
#    MinMaxScaler -> PCA(n_components=512) for AmplitudeEmbedding
...
def load_az23_pca512(
    ...
    Xtr = pca.fit_transform(Xtr).astype(np.float32)  # shape: (N, 512)
...
# ============================================================
# 2. Quantum circuit  — matches NN_AZ_amplitude_l2_LF.py exactly
...
def build_proto_cache(Xtr: np.ndarray,
    ...
    Instead of storing N~184k density matrices (~770 GB), only
    N_CLASSES=23 prototypes are stored (~96 MB).

    Prototypes are built from the FULL training set. At N=184k,
    the effect of poisoning on per-class means is negligible (<1%).
...
#     Keeps all 23 classes, samples up to QDETECT_SUBSET_PER_CLASS
...
# ============================================================
# 7. train() and test() — match NN_quid_az_angle_l2.py exactly
# ============================================================
...
# ============================================================
# 8. evaluation() — matches NN_quid_az_angle_l2.py exactly
# ============================================================
...
# ============================================================
# 10. CSV helpers — match NN_quid_az_angle_l2.py exactly
# ============================================================
def save_per_run_csv(metrics, filepath):
    """['Metric','Value'] format — matches NN_quid_az_angle_l2.py."""
...
def main():
    print("=" * 68)
    print(" QUID + Q-Detection | AZ-Class 23-family | 9-qubit Amplitude model (PCA->512) | 50 layers")
    print(" Optimisations: prototype cache (~96 MB) + multi-GPU parallelism")
    print("=" * 68)
    ...
    # ---- build/load prototype cache (~96 MB vs ~770 GB for full cache) ----
        ...
        for eps in EPS_LIST:
            for attack in ["quid", "random"]:
                for use_qdetect in [False, True]:
                    # Q-WAN J is O(N²): subsampled to QDETECT_SUBSET_PER_CLASS per class
                    if use_qdetect:
    ...
    print(f" Cache   → {RHO_PROTO_CACHE_FILE}  (~96 MB, prototype-only)")
    print(f"{'='*68}")
