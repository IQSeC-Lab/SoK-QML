"""
depo_quid_az_angle_l10.py
============================================================
Depolarizing-noise evaluation of models saved by NN_quid_az_angle_l2.py.

For every saved best-model checkpoint (3 runs × 3 eps × 2 attacks × 2 defenses
= 36 models total) this script:
  1. Rebuilds the QMLPAZ model with default.mixed + DepolarizingChannel
     (faster and more accurate than qiskit.aer for 9 qubits)
...
Output folder structure mirrors the QUID script but with "depo_" prefix:
  depo_eps0.1/  depo_eps0.3/  depo_eps0.5/
    ├── depo-qmlp-az-quid-run{r}-layer2_quid_eps{e}_{attack}_{def}.csv
    └── depo-qmlp-az-quid-run{r}-layer2_quid_eps{e}_{attack}_{def}_cm.png
  depo_runs/
    └── depo_quid_qdetection_AZ23_angle_l10_summary.csv
============================================================
"""
...
# ============================================================
# Config — must match NN_quid_az_angle_l2.py exactly
# ============================================================
N_QUBITS   = 9
N_LAYERS   = 10
N_CLASSES  = 23
...
ENCODING             = "Angle"
RHO_PROTO_CACHE_FILE = "rho_proto_cache.npy"
...
DEPO_SUMMARY_CSV = os.path.join(DEPO_RUN_DIR, "depo_quid_qdetection_AZ23_angle_l10_summary.csv")
...
class QMLPAZ(nn.Module):
    """Same architecture as NN_quid_az_angle_l2.py but with noisy device."""
    def __init__(self):
        super().__init__()
...
# ============================================================
# 3. Data — must match NN_quid_az_angle_l2.py exactly
...
# ============================================================
# 4. Poisoning replay — identical to NN_quid_az_angle_l2.py
...
# ============================================================
# 5. Evaluation — matches NN_quid_az_angle_l2.py exactly
# ============================================================
...
def main():
    print("=" * 68)
    print(f" Depolarizing Noise Evaluation | QUID AZ-Angle models | p={DEPO_P}")
    ...
    if not os.path.exists(RHO_PROTO_CACHE_FILE):
        raise FileNotFoundError(
            ...
            f"Run NN_quid_az_angle_l2.py first to build it.")
