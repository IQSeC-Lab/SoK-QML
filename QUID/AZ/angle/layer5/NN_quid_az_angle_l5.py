...
Optimisations over naive implementation:
  1. PROTOTYPE CACHE  (replaces full density-matrix cache)
     ...
     rho_proto_cache.npy, and reused across every QUID call.
  ...
  2. MULTI-GPU PARALLEL EXPERIMENTS
     ...
     GPUs are used, e.g.:
       CUDA_VISIBLE_DEVICES=0,1,2,3 python QUID_QDetection_AZ23_PCA9.py

Model architecture (identical to NN_angle_AZ_LF_l5.py):
  * Dataset  : AZ-Class FULL 23-family malware dataset
  * Features : MinMaxScaler -> PCA(n_components=9)
  * Qubits   : 9  |  Layers: 2
  ...
  * Epochs   : 30,  batch_size=64,  runs=3  (seeds 43,44,45)

Output folder structure:
  eps0.0/  eps0.1/  eps0.3/  eps0.5/
    ├── *.pt   (best-accuracy checkpoint)
    └── *.csv  (per-combo metrics, ['Metric','Value'] format)
  runs/
    └── quid_qdetection_AZ23_summary.csv
  rho_proto_cache.npy  (cached class prototypes, shape [23, 512, 512], ~96 MB)
============================================================
"""
...
ENCODING          = "Angle"
RHO_PROTO_CACHE_FILE = "rho_proto_cache.npy"  # cached class prototypes (23 x 512 x 512)

RUN_DIR     = "runs"
os.makedirs(RUN_DIR, exist_ok=True)
SUMMARY_CSV = os.path.join(RUN_DIR, "quid_qdetection_AZ23_summary.csv")
...
# e.g. 23 * 348 = 8004 samples → 8004x8004 J matrix (~512 MB) instead of 271 GB
QDETECT_SUBSET_PER_CLASS = 700   # ~8000 total, adjust lower if still OOM
...
# ============================================================
# 7. train() and test() — match NN_angle_AZ_LF_l5.py exactly
# ============================================================
...
# ============================================================
# 8. evaluation() — matches NN_angle_AZ_LF_l5.py exactly
# ============================================================
...
def save_per_run_csv(metrics, filepath):
    """['Metric','Value'] format — matches NN_angle_AZ_LF_l5.py."""
...
def main():
    print("=" * 68)
    print(" QUID + Q-Detection | AZ-Class 23-family | 9-qubit PCA model")
    ...
    # ---- build task list ----
    # All 3 runs × 4 eps × 2 attacks × 2 defenses = 48 tasks
    tasks = []
