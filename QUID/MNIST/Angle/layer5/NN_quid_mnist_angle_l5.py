"""
QUID_QDetection_MNIST_Angle_l2.py
============================================================
Combines:
...
Optimisations over naive implementation:
  1. PROTOTYPE CACHE  (replaces full density-matrix cache)
     ...
     Prototypes are computed ONCE at startup using all available CPU
     cores (joblib), saved to rho_proto_cache_mnist_angle.npy, and
     reused across every QUID call.  Re-computation is skipped on
     subsequent runs if the cache file already exists.
  ...
  2. MULTI-GPU PARALLEL EXPERIMENTS
     ...
     GPUs are used, e.g.:
       CUDA_VISIBLE_DEVICES=0,1,2,3 python NN_quid_mnist_angle_l2.py

Model architecture (identical to noiseless_angle_label_flipping_l2.py):
  * Dataset  : MNIST 10-class handwritten digits
  * Features : downscale(3x3) -> flatten -> MinMaxScaler -> PCA(n_components=9)
  * Qubits   : 9  |  Layers: 2
...
Output folder structure:
  ...
  runs/
    └── quid_qdetection_MNIST_angle_summary.csv
  rho_proto_cache_mnist_angle.npy  (cached class prototypes, shape [10, 512, 512])
============================================================
"""
...
SUMMARY_CSV = os.path.join(RUN_DIR, "quid_qdetection_MNIST_angle_summary.csv")
...
# e.g. 10 * 700 = 7000 samples → 7000x7000 J matrix (~392 MB) instead of 28.8 GB
QDETECT_SUBSET_PER_CLASS = 700   # ~7000 total, adjust lower if still OOM
...
# ============================================================
# 1. Data  — matches noiseless_angle_label_flipping_l2.py exactly
#    downscale(3x3) -> flatten -> MinMaxScaler -> PCA(9)
# ============================================================
def _downscale_images(X, new_size=(3, 3)):
    """Downscale MNIST 28x28 images to new_size (default 3x3 = 9 features)."""
...
#    Shape: (N_CLASSES, 512, 512) complex128.
...
def build_proto_cache(Xtr: np.ndarray,
    ...
    Instead of storing N=60k density matrices, only N_CLASSES=10
...
def quid_label_flip_with_data(Xtr, proto_cache, y, epsilon, seed=BASE_SEED):
    ...
    Class prototypes come from proto_cache (pre-built).

    This avoids storing N=60k density matrices on disk.
    """
    if epsilon <= 0.0:
...
# ============================================================
# 7. train() and test() — match noiseless_angle_label_flipping_l2.py exactly
# ============================================================
...
def main():
    print("=" * 68)
    print(" QUID + Q-Detection | MNIST 10-class | 9-qubit Angle model (PCA->9)")
