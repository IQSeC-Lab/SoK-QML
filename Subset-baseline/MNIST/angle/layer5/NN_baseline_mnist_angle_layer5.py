...
Layer config : 5 layers
Runs         : 1  (seed 43)
Epochs       : 30 (early stopping patience=5)
GPUs         : 1 MIG slice

Model architecture (identical to NN_quid_mnist_angle_l5.py):
  * Features  : downscale(3x3) -> flatten -> MinMaxScaler -> PCA(9)
  * Qubits    : 9
  * Encoding  : AngleEmbedding
  * PQC       : Rot(a,b,g) + cyclic CRX  x 5 layers
  * Post      : Linear(9->10) + log_softmax
  * Opt       : Adam lr=0.001, weight_decay=1e-4
  * Subset    : stratified 700/class (~7000 total)

Output structure:
  baseline_mnist_angle_layer5/
    baseline_mnist_angle_layer5_run1_epoch_metrics.csv
    baseline_mnist_angle_layer5_run1_metrics.csv
    baseline_mnist_angle_layer5_summary.csv
    best_model_run1.pt
============================================================
"""
...
OUT_DIR     = "baseline_mnist_angle_layer5"
SUMMARY_CSV = os.path.join(OUT_DIR, "baseline_mnist_angle_layer5_summary.csv")
...
# ============================================================
# 1. Data loading — matches NN_quid_mnist_angle_l5.py exactly
# ============================================================
...
def save_epoch_csv(epoch_recs, run_id):
    path = os.path.join(OUT_DIR,
                        f"baseline_mnist_angle_layer5_run{run_id}_epoch_metrics.csv")
...
def save_per_run_csv(metrics, run_id):
    path = os.path.join(OUT_DIR,
                        f"baseline_mnist_angle_layer5_run{run_id}_metrics.csv")
    with open(path, "w", newline="") as f:
...
def experiment_worker(task, Xtr, ytr, Xte, yte, csv_lock):
    ...
    tag      = f"[MNIST|Ang|L5|R{run_id}|GPU{gpu_id}]"
...
def main():
    global GPUS
    print(SEP)
    print(f"  Baseline | MNIST 10-class | 9-qubit | AngleEmbedding | 5 layers")
    ...
    print(f"  MNIST Angle | Layer 5 — mean ± std across {N_RUNS} runs:")
    ...
    print(f"  Epoch CSVs   -> {OUT_DIR}/baseline_mnist_angle_layer5_run*_epoch_metrics.csv")
    print(f"  Per-run CSVs -> {OUT_DIR}/baseline_mnist_angle_layer5_run*_metrics.csv")
    print(SEP)
