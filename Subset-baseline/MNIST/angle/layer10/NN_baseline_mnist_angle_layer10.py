"""
NN_baseline_mnist_angle_layer10.py
============================================================
...
Model architecture (identical to NN_quid_mnist_angle_l10.py):
...
Output structure:
  baseline_mnist_angle_layer10/
    baseline_mnist_angle_layer10_run1_epoch_metrics.csv
    baseline_mnist_angle_layer10_run1_metrics.csv
    baseline_mnist_angle_layer10_summary.csv
    best_model_run1.pt
============================================================
"""
...
OUT_DIR     = "baseline_mnist_angle_layer10"
SUMMARY_CSV = os.path.join(OUT_DIR, "baseline_mnist_angle_layer10_summary.csv")
...
# ============================================================
# 1. Data loading — matches NN_quid_mnist_angle_l10.py exactly
# ============================================================
...
def save_epoch_csv(epoch_recs, run_id):
    path = os.path.join(OUT_DIR,
                        f"baseline_mnist_angle_layer10_run{run_id}_epoch_metrics.csv")
...
def save_per_run_csv(metrics, run_id):
    path = os.path.join(OUT_DIR,
                        f"baseline_mnist_angle_layer10_run{run_id}_metrics.csv")
    with open(path, "w", newline="") as f:
...
def experiment_worker(task, Xtr, ytr, Xte, yte, csv_lock):
    ...
    tag      = f"[MNIST|Ang|L10|R{run_id}|GPU{gpu_id}]"
...
def main():
    global GPUS
    print(SEP)
    print(f"  Baseline | MNIST 10-class | 9-qubit | AngleEmbedding | 10 layers")
    ...
    print(f"  MNIST Angle | Layer 10 — mean ± std across {N_RUNS} runs:")
    ...
    print(f"  Epoch CSVs   -> {OUT_DIR}/baseline_mnist_angle_layer10_run*_epoch_metrics.csv")
    print(f"  Per-run CSVs -> {OUT_DIR}/baseline_mnist_angle_layer10_run*_metrics.csv")
    print(SEP)
