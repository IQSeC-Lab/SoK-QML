"""
NN_baseline_mnist_angle_layer2.py
============================================================
...
Model architecture (identical to NN_quid_mnist_angle_l2.py):
...
Output structure:
  baseline_mnist_angle_layer2/
    baseline_mnist_angle_layer2_run1_epoch_metrics.csv
    baseline_mnist_angle_layer2_run1_metrics.csv
    baseline_mnist_angle_layer2_summary.csv
    best_model_run1.pt
============================================================
"""
...
OUT_DIR     = "baseline_mnist_angle_layer2"
SUMMARY_CSV = os.path.join(OUT_DIR, "baseline_mnist_angle_layer2_summary.csv")
...
# ============================================================
# 1. Data loading — matches NN_quid_mnist_angle_l2.py exactly
# ============================================================
...
def save_epoch_csv(epoch_recs, run_id):
    path = os.path.join(OUT_DIR,
                        f"baseline_mnist_angle_layer2_run{run_id}_epoch_metrics.csv")
...
def save_per_run_csv(metrics, run_id):
    path = os.path.join(OUT_DIR,
                        f"baseline_mnist_angle_layer2_run{run_id}_metrics.csv")
    with open(path, "w", newline="") as f:
...
def experiment_worker(task, Xtr, ytr, Xte, yte, csv_lock):
    ...
    tag      = f"[MNIST|Ang|L2|R{run_id}|GPU{gpu_id}]"
...
def main():
    global GPUS
    print(SEP)
    print(f"  Baseline | MNIST 10-class | 9-qubit | AngleEmbedding | 2 layers")
    ...
    print(f"  MNIST Angle | Layer 2 — mean ± std across {N_RUNS} runs:")
    ...
    print(f"  Epoch CSVs   -> {OUT_DIR}/baseline_mnist_angle_layer2_run*_epoch_metrics.csv")
    print(f"  Per-run CSVs -> {OUT_DIR}/baseline_mnist_angle_layer2_run*_metrics.csv")
    print(SEP)
