"""
depo_baseline_mn_amp_l2.py
============================================================
Depolarizing-noise evaluation of the BASELINE model saved by
NN_baseline_mnist_amplitude_layer2.py.

NO attack, NO defense — evaluates how the clean baseline
model performs under depolarizing noise (p=0.01).

Steps:
  1. Rebuilds the amplitude-encoding model with
...
Saved model expected (from NN_baseline_mnist_amplitude_layer2.py):
  baseline_mnist_amplitude_layer2/best_model_run1.pt

Output folder structure:
  depo_baseline_amplitude/
    depo_baseline_amplitude_metrics.csv
    depo_baseline_amplitude_cm.png
  depo_baseline_runs/
    depo_baseline_mn_amp_l2_summary.csv
============================================================
"""
...
# ============================================================
# Config — must match NN_baseline_mnist_amplitude_layer2.py
# ============================================================
N_QUBITS          = 9
N_LAYERS          = 2
N_CLASSES         = 10
PCA_COMPONENTS    = 512        # AmplitudeEmbedding needs 2^9 = 512
BATCH_SIZE        = 1024
BASE_SEED         = 42
SUBSET_PER_CLASS  = 700

DEPO_P       = 0.01
DEPO_OUT_DIR = "depo_baseline_amplitude"
DEPO_RUN_DIR = "depo_baseline_runs"
DEPO_SUMMARY_CSV = os.path.join(
    DEPO_RUN_DIR, "depo_baseline_mn_amp_l2_summary.csv")

MODEL_CHECKPOINT = "baseline_mnist_amplitude_layer2/best_model_run1.pt"
...
print(f"[Config] N_LAYERS={N_LAYERS}  DEPO_P={DEPO_P}  PCA={PCA_COMPONENTS}")
...
#    AmplitudeEmbedding applied ONCE — DepolarizingChannel
#    after every Rot and CRX gate
# ============================================================
...
class NoisyBaseline(nn.Module):
    """Noisy amplitude-encoding QMLP — same architecture as baseline."""
    def __init__(self):
        super().__init__()
...
# ============================================================
# 3. Data — must match NN_baseline_mnist_amplitude_layer2.py
#    flatten(784) -> MinMaxScaler -> PCA(512)
# ============================================================
...
def save_cm(cm, filepath):
    ...
    ax.set_title(
        ...
        f"Baseline MNIST Amplitude L{N_LAYERS} | Clean model, noisy inference",
        fontsize=11)
...
def main():
    print("=" * 68)
    print(f" Depolarizing Noise Evaluation | Baseline MNIST Amplitude L{N_LAYERS}")
    ...
    # ── Load data ─────────────────────────────────────────────
    Xtr_full, ytr_full, Xte, yte = load_mnist_pca512()
    ...
    if not os.path.exists(MODEL_CHECKPOINT):
        raise FileNotFoundError(
            ...
            f"Run NN_baseline_mnist_amplitude_layer2.py first.")
    ...
    save_metrics_csv(metrics,
                     os.path.join(DEPO_OUT_DIR, "depo_baseline_amplitude_metrics.csv"))
    save_cm(cm,
            os.path.join(DEPO_OUT_DIR, "depo_baseline_amplitude_cm.png"))
