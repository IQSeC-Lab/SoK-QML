"""
NN_baseline_az_angle_layer2.py
============================================================
...
Launch via sbatch:
  sbatch job_layer2.sh

Output structure:
  baseline_layer2/
    baseline_layer2_run1_epoch_metrics.csv   (epoch | train_loss | train_acc_pct | test_acc_pct)
    baseline_layer2_run2_epoch_metrics.csv
    baseline_layer2_run3_epoch_metrics.csv
    baseline_layer2_run1_metrics.csv         (Metric | Value  per-run final)
    baseline_layer2_run2_metrics.csv
    baseline_layer2_run3_metrics.csv
    baseline_layer2_summary.csv              (all 3 runs, wide format)
============================================================
"""
...
OUT_DIR     = "baseline_layer2"
SUMMARY_CSV = os.path.join(OUT_DIR, "baseline_layer2_summary.csv")
...
def save_epoch_csv(epoch_recs, run_id):
    ...
    path = os.path.join(OUT_DIR,
                        f"baseline_layer2_run{run_id}_epoch_metrics.csv")
...
def save_per_run_csv(metrics, run_id):
    ...
    path = os.path.join(OUT_DIR,
                        f"baseline_layer2_run{run_id}_metrics.csv")
    with open(path, "w", newline="") as f:
...
def main():
    global GPUS
    print(SEP)
    print(f"  Baseline | AZ-Class 23-family | 9-qubit | 2 layers")
    ...
    print(f"  Layer 2 — mean ± std across {N_RUNS} runs:")
    ...
    print(f"  Epoch CSVs   -> {OUT_DIR}/baseline_layer2_run*_epoch_metrics.csv")
    print(f"  Per-run CSVs -> {OUT_DIR}/baseline_layer2_run*_metrics.csv")
    print(SEP)
