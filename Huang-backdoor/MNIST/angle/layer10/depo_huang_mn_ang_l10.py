...
ratios = 3 models total) this script:
  ...
  3. Reconstructs the poisoned dataset deterministically
     (same seeds as huang script) to replay trigger application
  4. Runs noisy inference on clean test set → full metrics (CA)
  5. Runs noisy inference on non-target test samples with
     trigger → ASR under noise
  6. Saves per-model metrics CSV + confusion matrix PNG +
     summary CSV
...
Saved models expected (from huang_bd_mn_ang_l2.py):
  huang_angle_best_ratio01.pt   ← poison ratio 0.1
  huang_angle_best_ratio03.pt   ← poison ratio 0.3
  huang_angle_best_ratio05.pt   ← poison ratio 0.5
  huang_angle_l10_trigger.pt     ← universal trigger tensor

Output folder structure:
  depo_huang_angle_l10/
    ├── depo_huang_angle_ratio{P}_metrics.csv
    └── depo_huang_angle_ratio{P}_cm.png
  depo_runs/
    └── depo_huang_mn_ang_l10_summary.csv
============================================================
"""
...
# ============================================================
# Config — must match huang_bd_mn_ang_l2.py exactly
# ============================================================
N_QUBITS          = 9
N_LAYERS          = 10
N_CLASSES         = 10
...
DEPO_P       = 0.01
DEPO_OUT_DIR = "depo_huang_angle_l10"
DEPO_RUN_DIR = "depo_runs"
DEPO_SUMMARY_CSV = os.path.join(
    DEPO_RUN_DIR, "depo_huang_mn_ang_l10_summary.csv")
...
print(f"[Config] N_LAYERS={N_LAYERS}  DEPO_P={DEPO_P}  TARGET_CLASS={TARGET_CLASS}")
...
#    every Rot and CRX gate (same circuit as training script)
# ============================================================
...
class drebin(nn.Module):
    """Noisy angle-encoding QMLP — same architecture as training script."""
    def __init__(self):
        super().__init__()
...
# ============================================================
# 3. Data — must match huang_bd_mn_ang_l2.py exactly
...
def compute_asr(model, Xte, yte, trigger):
    """
    Apply trigger to all non-target test samples and measure
    what fraction is predicted as TARGET_CLASS.
    """
    model.eval()
...
def model_path(poison_ratio):
    return f"huang_angle_best_ratio{int(poison_ratio*10):02d}.pt"

def out_csv_path(poison_ratio):
    return os.path.join(
        DEPO_OUT_DIR,
        f"depo_huang_angle_ratio{int(poison_ratio*10):02d}_metrics.csv")

def out_cm_path(poison_ratio):
    return os.path.join(
        DEPO_OUT_DIR,
        f"depo_huang_angle_ratio{int(poison_ratio*10):02d}_cm.png")
...
def save_cm(cm, poison_ratio, filepath):
    ...
    ax.set_title(
        ...
        f"Huang Backdoor MNIST Angle L{N_LAYERS} | "
...
def main():
    print("=" * 68)
    print(f" Depolarizing Noise Evaluation | Huang Backdoor MNIST Angle L{N_LAYERS}")
    ...
    # ── Load trigger ──────────────────────────────────────────────────────
    trigger_path = "huang_angle_l10_trigger.pt"
    if not os.path.exists(trigger_path):
        raise FileNotFoundError(
            f"Trigger not found: {trigger_path}\n"
            f"Run huang_bd_mn_ang_l2.py first.")
