...
Paper fidelity:
  - Algorithm 1: per-sample sequential delta accumulation      [CORRECT]
...
Output files (all in runs/):
  runs/huang_angle_l10_summary.csv
  runs/huang_proxy_epoch_metrics.csv
  runs/huang_angle_run1_ratio{P}_epoch_metrics.csv
  runs/huang_angle_run1_ratio{P}_final_metrics.csv
  huang_angle_l10_trigger.pt
  huang_proxy_angle_l10_best.pt
  huang_angle_best_ratio{P}.pt
============================================================
"""
...
SUMMARY_CSV = os.path.join(RUN_DIR, "huang_angle_l10_summary.csv")
...
# ============================================================
# 2. Quantum model — angle encoding (re-uploading style)
#    Matches NN_quid_mnist_angle_l2.py exactly
# ============================================================
...
class drebin(nn.Module):
    """
    Angle-encoding QMLP.
    Matches NN_quid_mnist_angle_l2.py (QMLPMNIST class).
    Each instance owns its device + qnode.
    """
    def __init__(self):
        super().__init__()
        dev         = make_device()
...
# ============================================================
# 3. Data helpers — matches NN_quid_mnist_angle_l2.py exactly
#    downscale(3x3) -> flatten -> MinMaxScaler -> PCA(9)
# ============================================================
...
def victim_run_worker(args):
    ...
    tmp_ckpt  = f"huang_angle_tmp_{ratio_tag}.pt"
    ...
    epoch_csv = os.path.join(
        RUN_DIR, f"huang_angle_{ratio_tag}_epoch_metrics.csv")
    ...
    final_csv = os.path.join(
        RUN_DIR, f"huang_angle_{ratio_tag}_final_metrics.csv")
...
def main():
    ...
    print("=" * 68)
    print(" Huang & Zhang (2023) Backdoor — Angle QMLP Replication")
    print(f" Qubits={N_QUBITS}  Layers={N_LAYERS}  Encoding=Angle")
    print(f" Target={TARGET_CLASS}  eps={EPS}  "
          f"(c,sigma)=({FUZZY_C},{FUZZY_SIGMA})")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if test_acc > best_proxy_acc:
            ...
            torch.save(proxy.state_dict(), "huang_proxy_angle_l10_best.pt")
            es_cnt = 0
        else:
            es_cnt += 1
            if es_cnt >= ES_PATIENCE:
    ...
    proxy_epoch_csv = os.path.join(RUN_DIR, "huang_proxy_epoch_metrics.csv")
    save_epoch_csv(proxy_epoch_csv, proxy_epoch_rows)

    proxy.load_state_dict(
        torch.load("huang_proxy_angle_l10_best.pt", map_location=MAIN_DEVICE))
    ...
    trigger = generate_trigger(proxy, Xtr_sub[nt_samp], Xtr_sub[t_idx],
                               MAIN_DEVICE)
    torch.save(trigger, "huang_angle_l10_trigger.pt")
    ...
    for poison_ratio in POISON_RATIOS:
        ...
        # save best model for this ratio
        import shutil
        best_result   = max(results, key=lambda r: r["_best_ca"])
        best_ckpt_dst = f"huang_angle_best_ratio{int(poison_ratio*10):02d}.pt"
