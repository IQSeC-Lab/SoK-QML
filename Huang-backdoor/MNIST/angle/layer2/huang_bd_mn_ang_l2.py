...
Output files (all in runs/):
  runs/huang_angle_l2_summary.csv
  ...
  huang_angle_l2_trigger.pt
  huang_proxy_angle_l2_best.pt
...
SUMMARY_CSV = os.path.join(RUN_DIR, "huang_angle_l2_summary.csv")
...
def main():
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if test_acc > best_proxy_acc:
            ...
            torch.save(proxy.state_dict(), "huang_proxy_angle_l2_best.pt")
            es_cnt = 0
        else:
            es_cnt += 1
            if es_cnt >= ES_PATIENCE:
    ...
    proxy.load_state_dict(
        torch.load("huang_proxy_angle_l2_best.pt", map_location=MAIN_DEVICE))
    ...
    trigger = generate_trigger(proxy, Xtr_sub[nt_samp], Xtr_sub[t_idx],
                               MAIN_DEVICE)
    torch.save(trigger, "huang_angle_l2_trigger.pt")
