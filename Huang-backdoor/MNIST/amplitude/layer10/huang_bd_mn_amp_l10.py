...
Model  : exact drebin from noiseless_amplitude_label_flipping_layer2.py
         9 qubits | 10 layers | AmplitudeEmbedding
         Rot(φ,θ,ω) + cyclic CRX × 10 | Linear(9→10) | log_softmax
...
# ============================================================
# 1.  Quantum device  — lightning.qubit uses all CPU cores
# ============================================================
def make_device():
    """
    Try lightning.qubit first (fast C++ OpenMP simulator).
    Fall back to default.qubit if not installed.
    """
    try:
        dev = qml.device("lightning.qubit", wires=N_QUBITS)
        print("[PennyLane] Using lightning.qubit (install: pip install pennylane-lightning)")
        return dev
    except Exception:
        print("[PennyLane] lightning.qubit not found — falling back to default.qubit")
        print("            Run:  pip install pennylane-lightning  for ~10-20× speedup")
...
class drebin(nn.Module):
    """
    Amplitude-encoding QMLP — 10 layers.
...
def main():
    ...
    # ── Phase 1: Train proxy on clean data (main process, GPU if avail) ─
    print("\n" + "="*68)
    print(" PHASE 1 — Train proxy model (clean, seed=42)")
    print("="*68)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if test_acc > best_proxy_acc:
            ...
            torch.save(proxy.state_dict(), "huang_proxy_best.pt")
            es_cnt = 0
        else:
            es_cnt += 1
            if es_cnt >= ES_PATIENCE:
