...
Output files (all in runs/):
  runs/huang_az_amp_l2_summary.csv
  runs/huang_az_amp_proxy_l2_epoch_metrics.csv
  runs/huang_az_amp_l2_run1_ratio{P}_epoch_metrics.csv
  runs/huang_az_amp_l2_run1_ratio{P}_final_metrics.csv
  huang_az_amp_l2_trigger.pt
  huang_az_amp_proxy_l2_best.pt
  huang_az_amp_l2_best_ratio{P}.pt
============================================================
"""
...
SUMMARY_CSV = os.path.join(RUN_DIR, "huang_az_amp_l2_summary.csv")
...
def victim_run_worker(args):
    ...
    tmp_ckpt  = f"huang_az_amp_l2_tmp_{ratio_tag}.pt"
    ...
    epoch_csv = os.path.join(
        RUN_DIR, f"huang_az_amp_l2_{ratio_tag}_epoch_metrics.csv")
    ...
    final_csv = os.path.join(
        RUN_DIR, f"huang_az_amp_l2_{ratio_tag}_final_metrics.csv")
...
def main():
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if test_acc > best_proxy_acc:
            ...
            torch.save(proxy.state_dict(), "huang_az_amp_proxy_l2_best.pt")
            es_cnt = 0
        else:
            es_cnt += 1
            if es_cnt >= ES_PATIENCE:
    ...
    proxy_epoch_csv = os.path.join(
        RUN_DIR, "huang_az_amp_proxy_l2_epoch_metrics.csv")
    save_epoch_csv(proxy_epoch_csv, proxy_epoch_rows)
    proxy.load_state_dict(
        torch.load("huang_az_amp_proxy_l2_best.pt", map_location=MAIN_DEVICE))
    ...
    torch.save(trigger, "huang_az_amp_l2_trigger.pt")
    ...
    for poison_ratio in POISON_RATIOS:
        ...
        best_ckpt_dst = f"huang_az_amp_l2_best_ratio{int(poison_ratio*10):02d}.pt"
