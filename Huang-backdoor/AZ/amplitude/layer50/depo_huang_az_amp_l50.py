...
ratios = 3 models total) this script:
  1. Rebuilds the amplitude-encoding drebin model with
...
Noise backend: default.mixed + inline DepolarizingChannel
  ...
  - AmplitudeEmbedding uses broadcast_expand for batched inputs

Saved models expected (from huang_bd_az_amp_l2.py):
  huang_az_amp_l2_best_ratio01.pt   ← poison ratio 0.1
  huang_az_amp_l2_best_ratio03.pt   ← poison ratio 0.3
  huang_az_amp_l2_best_ratio05.pt   ← poison ratio 0.5
  huang_az_amp_l2_trigger.pt        ← universal trigger tensor

Output folder structure:
  depo_huang_az_amp_l50/
    ├── depo_huang_az_amp_l2_ratio{P}_metrics.csv
    └── depo_huang_az_amp_l2_ratio{P}_cm.png
  depo_runs/
    └── depo_huang_az_amp_l50_summary.csv
============================================================
"""
...
# ============================================================
# Config — must match huang_bd_az_amp_l2.py exactly
# ============================================================
N_QUBITS          = 9
N_LAYERS          = 50
N_CLASSES         = 23
...
TEST_SUBSET_PER_CLASS = 200  # 200/class x 23 = 4,600 test samples
PCA_COMPONENTS    = 512     # 2^9 = 512 for AmplitudeEmbedding

DEPO_P       = 0.01
DEPO_OUT_DIR = "depo_huang_az_amp_l50"
DEPO_RUN_DIR = "depo_runs"
DEPO_SUMMARY_CSV = os.path.join(
    DEPO_RUN_DIR, "depo_huang_az_amp_l50_summary.csv")
...
print(f"[Config] PCA_COMPONENTS={PCA_COMPONENTS} (AmplitudeEmbedding)")
...
# ============================================================
# 2. Noisy quantum model — AmplitudeEmbedding + DepolarizingChannel
#    broadcast_expand needed for batched AmplitudeEmbedding inputs
# ============================================================
...
class drebin(nn.Module):
    """Noisy amplitude-encoding QMLP for AZ-23. Linear(9->23)."""
    def __init__(self):
        super().__init__()
...
# ============================================================
# 3. Data — must match huang_bd_az_amp_l2.py exactly
...
# ============================================================
# 3b. Stratified test subset — 200 samples per class
#     Reduces evaluation from ~46k to 4,600 samples (~10x faster)
# ============================================================
def stratified_test_subset(X, y, n_per_class, seed=BASE_SEED):
...
def model_path(poison_ratio):
    return f"huang_az_amp_l{N_LAYERS}_best_ratio{int(poison_ratio*10):02d}.pt"

def out_csv_path(poison_ratio):
    return os.path.join(
        DEPO_OUT_DIR,
        f"depo_huang_az_amp_l{N_LAYERS}_ratio{int(poison_ratio*10):02d}_metrics.csv")

def out_cm_path(poison_ratio):
    return os.path.join(
        DEPO_OUT_DIR,
        f"depo_huang_az_amp_l{N_LAYERS}_ratio{int(poison_ratio*10):02d}_cm.png")
...
def save_cm(cm, poison_ratio, filepath):
    ...
    ax.set_title(
        ...
        f"Huang Backdoor AZ-23 Amplitude L{N_LAYERS} | "
...
def main():
    print("=" * 68)
    print(f" Depolarizing Noise Evaluation | Huang Backdoor AZ-23 Amplitude L{N_LAYERS}")
    print(f" Noise backend : default.mixed + DepolarizingChannel (p={DEPO_P})")
    print(f" Encoding      : AmplitudeEmbedding (PCA={PCA_COMPONENTS})")
    print(f" Models        : {len(POISON_RATIOS)} victim models (1 per ratio)")
    print(f" Target class  : {TARGET_CLASS} (7th class, 0-indexed)")
    print("=" * 68)

    # ── Load data ─────────────────────────────────────────────────────────
    _, _, Xte_full, yte_full = load_az23_amplitude()
    Xte, yte = stratified_test_subset(
        Xte_full, yte_full, TEST_SUBSET_PER_CLASS, seed=BASE_SEED)
    test_loader = make_loader(Xte, yte, shuffle=False)
    print(f"[Data] Full test: {Xte_full.shape}  "
          f"Stratified subset: {Xte.shape} "
          f"({TEST_SUBSET_PER_CLASS}/class x {N_CLASSES} classes)")

    # ── Load trigger ──────────────────────────────────────────────────────
    trigger_path = f"huang_az_amp_l{N_LAYERS}_trigger.pt"
    if not os.path.exists(trigger_path):
        raise FileNotFoundError(
            f"Trigger not found: {trigger_path}\n"
            f"Run huang_bd_az_amp_l{N_LAYERS}.py first.")
