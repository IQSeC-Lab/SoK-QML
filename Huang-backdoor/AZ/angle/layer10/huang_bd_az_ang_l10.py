...
Model  : angle-encoding QMLP from NN_quid_az_angle_l2.py
         9 qubits | 2 layers | AngleEmbedding (re-uploading)
         [AngleEmbedding -> Rot(phi,theta,omega) -> cyclic CRX] x 2
         Linear(9->23) | log_softmax

Dataset: AZ-Class 23-family malware dataset
         MinMaxScaler -> PCA(9)
         Full training set used (no stratified subsampling)
...
Attack (Huang & Zhang §4):
  ...
  4. Train victim model on poisoned data — 1 run per ratio      §5.1
       - max 30 epochs, early stopping patience=3
  5. Report CA and ASR per poison ratio                         §3.3

Output files (all in runs/):
  runs/huang_az_ang_l10_summary.csv
  runs/huang_az_proxy_epoch_metrics.csv
  runs/huang_az_ang_run1_ratio{P}_epoch_metrics.csv
  runs/huang_az_ang_run1_ratio{P}_final_metrics.csv
  huang_az_ang_l10_trigger.pt
  huang_az_proxy_ang_l10_best.pt
  huang_az_ang_l10_best_ratio{P}.pt
============================================================
"""
...
SAMPLES_PER_CLASS = 700    # stratified subset: 700 per class = 16,100 total

PCA_COMPONENTS    = N_QUBITS   # angle encoding: 1 feature per qubit

RUN_DIR     = "runs"
os.makedirs(RUN_DIR, exist_ok=True)
SUMMARY_CSV = os.path.join(RUN_DIR, "huang_az_ang_l10_summary.csv")
...
def make_device():
    try:
        ...
        print("[PennyLane] lightning.qubit active")
        return dev
    except Exception:
        print("[PennyLane] falling back to default.qubit")
        return qml.device("default.qubit", wires=N_QUBITS)


# ============================================================
# 2. Quantum model — matches NN_quid_az_angle_l2.py (QMLPAZ)
#    AngleEmbedding re-uploading inside each layer
# ============================================================
...
class drebin(nn.Module):
    """
    Angle-encoding QMLP for AZ-23 dataset.
    Matches NN_quid_az_angle_l2.py (QMLPAZ class).
    Linear(9->23) output layer for 23 malware families.
    """
    def __init__(self):
        super().__init__()
        dev         = make_device()
...
# ============================================================
# 3. Data loading — matches NN_quid_az_angle_l2.py exactly
#    MinMaxScaler -> PCA(9), no image preprocessing needed
# ============================================================
def load_az23_angle(
...
# ============================================================
# 3b. Stratified subset — 700 samples per class
#     Matches MNIST Huang scripts exactly
# ============================================================
...
def victim_run_worker(args):
    ...
    tmp_ckpt  = f"huang_az_ang_l10_tmp_{ratio_tag}.pt"
    ...
    epoch_csv = os.path.join(
        RUN_DIR, f"huang_az_ang_{ratio_tag}_epoch_metrics.csv")
    ...
    final_csv = os.path.join(
        RUN_DIR, f"huang_az_ang_{ratio_tag}_final_metrics.csv")
...
def main():
    ...
    print("=" * 68)
    print(" Huang & Zhang (2023) Backdoor — AZ-23 Angle QMLP")
    ...
    print("\n" + "="*68)
    print(" PHASE 1 — Proxy model training (clean, seed=42)")
    print("="*68)
    ...
    proxy_opt = optim.Adam(proxy.parameters(), lr=LR, weight_decay=W_DECAY)
    proxy_ds  = TensorDataset(
        torch.tensor(Xtr_sub, dtype=torch.float32),
        torch.tensor(ytr_sub, dtype=torch.long))
    proxy_loader = DataLoader(proxy_ds, shuffle=True, batch_size=BATCH_SIZE)
    ...
    for epoch in range(1, EPOCHS + 1):
        train_acc,  train_loss = train_epoch(
            proxy, proxy_loader, proxy_opt, epoch, MAIN_DEVICE)
        test_acc,   test_loss  = evaluate_ca_loss(
            proxy, test_loader, MAIN_DEVICE)

        proxy_epoch_rows.append({
            "epoch"      : epoch,
            "train_loss" : round(train_loss, 6),
            "train_acc"  : round(train_acc,  4),
            "test_loss"  : round(test_loss,  6),
        ...
        print(f"  [Proxy] Epoch {epoch:>2}  "
              f"tr_loss={train_loss:.4f}  tr_acc={train_acc:.2f}%  "
              f"te_loss={test_loss:.4f}  te_acc={test_acc:.2f}%")

        if test_acc > best_proxy_acc:
            best_proxy_acc = test_acc
            torch.save(proxy.state_dict(), "huang_az_proxy_ang_l10_best.pt")
            es_cnt = 0
        else:
            es_cnt += 1
            if es_cnt >= ES_PATIENCE:
                print(f"  [Proxy] Early stopping at epoch {epoch}")
                break

    proxy_epoch_csv = os.path.join(RUN_DIR, "huang_az_proxy_epoch_metrics.csv")
    save_epoch_csv(proxy_epoch_csv, proxy_epoch_rows)

    proxy.load_state_dict(
        torch.load("huang_az_proxy_ang_l10_best.pt", map_location=MAIN_DEVICE))
    print(f"  [Proxy] Best test CA: {best_proxy_acc:.2f}%")
    ...
    rng     = np.random.default_rng(BASE_SEED)
    # cap at 500 non-target samples for trigger generation speed
    nt_samp = rng.choice(nt_idx, size=min(500, len(nt_idx)), replace=False)
    # cap target samples at 500 for fuzzy admix pool
    ...
    trigger = generate_trigger(proxy, Xtr_sub[nt_samp], Xtr_sub[t_samp], MAIN_DEVICE)
    torch.save(trigger, "huang_az_ang_l10_trigger.pt")
    ...
    for poison_ratio in POISON_RATIOS:
        ...
        best_ckpt_dst = f"huang_az_ang_l10_best_ratio{int(poison_ratio*10):02d}.pt"
