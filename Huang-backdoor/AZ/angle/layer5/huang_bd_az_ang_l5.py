...
Output files (all in runs/):
  runs/huang_az_ang_l5_summary.csv
  ...
  huang_az_ang_l5_trigger.pt
  huang_az_proxy_ang_l5_best.pt
  huang_az_ang_l5_best_ratio{P}.pt
============================================================
"""
...
SUMMARY_CSV = os.path.join(RUN_DIR, "huang_az_ang_l5_summary.csv")
...
def victim_run_worker(args):
    ...
    tmp_ckpt  = f"huang_az_ang_l5_tmp_{ratio_tag}.pt"
...
def main():
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if test_acc > best_proxy_acc:
            ...
            torch.save(proxy.state_dict(), "huang_az_proxy_ang_l5_best.pt")
            es_cnt = 0
        else:
            es_cnt += 1
            if es_cnt >= ES_PATIENCE:
    ...
    proxy.load_state_dict(
        torch.load("huang_az_proxy_ang_l5_best.pt", map_location=MAIN_DEVICE))
    ...
    torch.save(trigger, "huang_az_ang_l5_trigger.pt")
    ...
    for poison_ratio in POISON_RATIOS:
        ...
        best_ckpt_dst = f"huang_az_ang_l5_best_ratio{int(poison_ratio*10):02d}.pt"
