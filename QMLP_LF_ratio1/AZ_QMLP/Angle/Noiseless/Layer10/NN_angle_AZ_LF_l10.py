...
best_model_prefix = "qmlp-az-label-flip-run"
filename_prefix = "qmlp-az-label-flip-run"
...
# Untargeted Label Flipping Function
...
print(f"Number of classes: {num_classes}")  # Should be 23

# Apply Untargeted Label Flipping
y_train = flip_labels(y_train, num_classes=num_classes, flip_fraction=0.01)
...
# Training and Testing (modified to return metrics)
...
def test(model, DEVICE, test_loader):
    ...
    print(f"Accuracy on test set: {acc:.2f}%, Val loss: {val_loss:.4f}")
    return acc, val_loss
...
for run_id in range(1, 4):
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(f"Epoch {epoch}: val_acc={acc:.2f}, best_val_acc={early_stopper.best_acc:.2f}, es_counter={early_stopper.counter}")
        if stop:
    ...
    # Restore best model according to early stopper
