...
filename_prefix = "qmlp-az-labelflip"  # adjust base name if needed
noise_afterfix = f"depo-LF_LS{n_qubits}-{n_layers}"
...
model.load_state_dict(torch.load("qmlp-az-amplitude-label-flip-run_best_run.pt", map_location=device))
...
# Save results
with open(f"{filename_prefix}_{noise_afterfix}_eval_results.csv", "w", newline="") as f:
    writer = csv.writer(f)
...
plt.title(f"Confusion Matrix with Depolarizing Noise (p={p})")
plt.xlabel("Predicted")
plt.ylabel("True")
plt.tight_layout()
plt.savefig(f"{filename_prefix}_{noise_afterfix}_confusion_matrix.png", dpi=300)
plt.close()

print(f"Done | Acc: {acc:.4f}, F1: {f1:.4f}, Avg FNR: {avg_fnr:.4f}, Avg FPR: {avg_fpr:.4f}")
