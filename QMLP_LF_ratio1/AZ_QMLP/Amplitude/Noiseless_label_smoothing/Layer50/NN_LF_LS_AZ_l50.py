...
# Training loop with label smoothing (returns avg epoch loss)
...
# Evaluation (returns loss as well)
def evaluate(model, device, loader, num_classes):
    ...
    return acc, prec, f1, cm, fnr_list, fpr_list, np.mean(fnr_list), np.mean(fpr_list), val_loss
...
# Label flipping
def flip_labels(y, num_classes, flip_fraction=0.01, seed=42):
    np.random.seed(seed)
    y_flipped = y.copy()
    n_samples = len(y)
...
for run_id in range(1, 4):
    ...
    early_stopper = EarlyStoppingAccuracy(patience=5, min_delta=0.01)
    test_accuracies = []
    
    for epoch in range(1, epochs + 1):
        train_loss = train(model, device, train_loader, optimizer, epoch, num_classes)
        acc, prec, f1, _, _, _, _, _, val_loss = evaluate(model, device, test_loader, num_classes)
        ...
        stop = early_stopper.step(acc, model)
        print(f"Epoch {epoch}: val_acc={acc:.4f}, best_val_acc={early_stopper.best_acc:.4f}, es_counter={early_stopper.counter}")
        if stop:
            print(f"Early stopping triggered at epoch {epoch}")
            break
    
    # Restore best model for this run
    early_stopper.restore_best(model)
    
    acc, prec, f1, cm, fnr_list, fpr_list, avg_fnr, avg_fpr, _ = evaluate(model, device, test_loader, num_classes)
...
# Plot test accuracy (handle variable length due to early stopping)
plt.plot(range(1, len(best_test_accuracies) + 1), best_test_accuracies, marker='o')
