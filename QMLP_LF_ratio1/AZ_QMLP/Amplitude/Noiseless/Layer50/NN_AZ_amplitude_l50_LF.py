...
class debrin(nn.Module):
    def __init__(self):
        super().__init__()
        self.qlayer = qlayer
        self.fc = nn.Linear(n_qubits, 23)  # 23 families
...
def flip_labels(y, num_classes=23, flip_fraction=0.01, seed=42):
    np.random.seed(seed)
    y_flipped = y.copy()
    n_samples = len(y)
...
y_train = flip_labels(y_train, num_classes,flip_fraction=0.01)
