...
lr = 0.001
w_decay = 1e-4
label_smoothing = 0.2  # Configurable smoothing factor
...
# Train function (with label smoothing, returns epoch loss)
...
# Label flipping
def flip_labels(y, num_classes=10, flip_fraction=0.01, seed=42):
    np.random.seed(seed)
    y_flipped = y.copy()
    n_samples = len(y)
...
# Run 3 experiments with early stopping
best_acc = 0
best_run_id = None
best_cm = None
best_fnr_list = None
