...
noise_afterfix = f"depo-testing{n_qubits}-{n_layers}"
...
# Load pretrained model and evaluate under noise
model = drebin().to(device)
model.load_state_dict(torch.load("qmlp-mnist-amplitude-label-flip-run_best_run.pt", map_location=device))
model.eval()
