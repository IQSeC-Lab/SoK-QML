...
parser.add_argument(
    "-l", "--layers",type=int,default=1,
    help="Number of layers in the QML model (default: 1)"
)
args = parser.parse_args()

####################################################################################################
# Extra for naming when using multiple datasets
print(f"using layers {args.layers}")
best_model_prefix = "run"
cm_name_prefix= "cm_run"
filename_prefix = "run"
noise_afterfix = ""
encoding = "Amplitude"
...
####################################################################################################
"""
Test and Training functions for the model
"""
...
def pgd_attack(model, X, y, epsilon, alpha, num_iter, random_start):
    ...
    if random_start:
        random_noise = (torch.rand_like(X) * 2 * epsilon - epsilon) 
...
# Devices
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu') # PyTorch Device - Classical
dev = qml.device("default.qubit", wires=n_qubits) # Pennylane Device - Quantum
###################################################################################
# Load the dataset
###################################################################################
# Change the path to load your dataset
# data_train = np.load("../Ember-Class_4_families_train.npz") 
# data_test = np.load("../Ember-Class_4_families_test.npz") 
...
# pca = PCA(n_components=n_qubits) # This is for Angle Encoding
...
# Main circuit for Amplitude Encoding
...
# Main Circuit for Angle Encoding
# @qml.qnode(dev, interface="torch")
# def qnode(inputs,**weights_kwargs):
#     for n in range(n_layers):
        ...
        for current_attack in attack_types_to_evaluate :
            ...
            for eps in epsilons:
                ...
                metrics = evaluation(
                    ...
                    attack_type=current_attack,pgd_alpha=PGD_ALPHA,         
                    pgd_num_iter=PGD_NUM_ITER,pgd_random_start=PGD_RANDOM_START 
                )
                results[eps] = metrics
