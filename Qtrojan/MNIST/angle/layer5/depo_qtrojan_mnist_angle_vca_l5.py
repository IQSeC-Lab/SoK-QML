"""
depo_qtrojan_mnist_angle_l5.py
============================================================
Depolarizing-noise evaluation of models saved by
qtrojan_mnist_angle_l5.py.

For the saved best-model checkpoint this script:
  1. Rebuilds QTrojanModel with default.mixed +
...
Dataset / preprocessing — matches qtrojan_mnist_angle_l5.py exactly:
  * MNIST 10-class
  * downscale(3×3) → flatten → MinMaxScaler → PCA(9)

Source model path:
  qtrojan/qtrojan-layer5_noiseless_Angle_best.pt

Output folder structure:
  depo_qtrojan/
    ├── depo_qtrojan_mnist_layer5_noiseless_Angle_metrics.csv
    └── depo_qtrojan_mnist_layer5_noiseless_Angle_cm.png
  depo_runs/
    └── depo_qtrojan_mnist_angle_summary.csv
============================================================
"""
...
# ============================================================
# Config — must match qtrojan_mnist_angle_l5.py exactly
# ============================================================
N_QUBITS   = 9
N_LAYERS   = 5
N_CLASSES  = 10
...
# Source model — saved by qtrojan_mnist_angle_l5.py
...
DEPO_SUMMARY_CSV = os.path.join(
    DEPO_RUN_DIR, "depo_qtrojan_mnist_angle_summary.csv")

os.makedirs(DEPO_OUT_DIR, exist_ok=True)
os.makedirs(DEPO_RUN_DIR, exist_ok=True)

device = torch.device("cpu")  # default.mixed runs on CPU — GPU causes OOM
...
#
#    DISABLED (backdoor off): AngleEmbedding → Rot+Depo+CRX+Depo
#    ENABLED  (backdoor on) : S̄x + Sx + S̃x → Rot+Depo+CRX+Depo
...
class QTrojanModel(nn.Module):
    """QTrojanModel with default.mixed + DepolarizingChannel."""
    def __init__(self):
        super().__init__()
...
# ============================================================
# 3. Data — must match qtrojan_mnist_angle_l5.py exactly
# ============================================================
...
def load_mnist_pca9(
    train_images="/home/vcadena1/work/Nowmi/Sok/Qtrojan/MNIST/MNIST/train-images-idx3-ubyte",
    train_labels="/home/vcadena1/work/Nowmi/Sok/Qtrojan/MNIST/MNIST/train-labels-idx1-ubyte",
    test_images ="/home/vcadena1/work/Nowmi/Sok/Qtrojan/MNIST/MNIST/t10k-images-idx3-ubyte",
    test_labels ="/home/vcadena1/work/Nowmi/Sok/Qtrojan/MNIST/MNIST/t10k-labels-idx1-ubyte",
):
...
def save_confusion_matrix(cm, filepath):
    ...
    ax.set_title(
        ...
        f"QTrojan MNIST | Angle | Layer{N_LAYERS} | Backdoor DISABLED",
        fontsize=11
    )
...
def main():
    ...
    if not os.path.exists(SOURCE_MODEL_PATH):
        raise FileNotFoundError(
            ...
            f"Run qtrojan_mnist_angle_l5.py first.")

    # ---- build noisy model and load weights ----
    print(f"\n[Model] Loading weights from {SOURCE_MODEL_PATH} ...")
    model = QTrojanModel().to(device)
    model.load_state_dict(torch.load(SOURCE_MODEL_PATH, map_location=device, weights_only=False))
    model.eval()
