...
def train_epoch_noiseless(params, Xtr8, ytr, qnode, tr: TrainConfig, opt: AdamNoiseless):
    ...
    losses = []
    for Xb, yb in make_minibatches(Xtr8, ytr, tr.batch_size, shuffle=True):
...
def train_epoch_noiseless_qd(params, Xtr8, ytr, qnode, tr: TrainConfig,
                             opt: AdamNoiseless,
                             qwan_bank: QWANIsingBank):
    theta, W, b = params
    losses_epoch = []
    for Xb, yb, idxb in make_minibatches(Xtr8, ytr, tr.batch_size, shuffle=True):
        # Stage A: AF — update persistent Q-WAN on this batch subgraph
        per_losses = compute_per_sample_losses(qnode, [theta, W, b], Xb, yb)
        # a couple of AF inner steps help stabilize early in training
        for _ in range(2):
            qwan_bank.train_step(idxb, per_losses)
        # Obtain weights for SL/AU
        v = qwan_bank.weights(idxb, per_losses)

        # Stage B/C: one weighted gradient step on (θ,W,b)
        ...
        # Log unweighted loss (for comparability)
        losses_epoch.append(float(xent_loss(logits_from_embed(
            pnp.stack([qnode(theta, Xb[i]) for i in range(len(yb))], axis=0), W, b
        ), yb)))
...
def main():
    # NOISY-ONLY split of QUID-8.py
    run_one_setting_pqc6(noisy=True, eps_list=EPS_LIST)
